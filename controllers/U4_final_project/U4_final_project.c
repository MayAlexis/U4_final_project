...
angle_init,angle_initi, posi_sensor,error_motor1, error_motor2, pid_term2;

/*
 * desireposition maximum value 3.14 minimum 0
 desire position1 minimum 0
 */ 
int desiredposition =0, desiredposition1=1.57, desiredposition3 = 0, desiredposition4 = -1, desiredposition5 = 3, desiredposition6 = 3;
...
int controlPID1(WbDeviceTag posi_sensor) {
  int angle_initi = wb_position_sensor_get_value(posi_sensor);
  int angle;
  
  error = desiredposition1 - angle_initi;
  angle = error - Last_error;
  Total_error += error;
  pid_term1 = (kp*error) + (ki*Total_error) + (kd*Change_error);
  pid_term1 = fabs(pid_term1);
  Last_error = error;
...
int main(int argc, char **argv)
{
  ...
  while (wb_robot_step(TIME_STEP) != -1) {
    ...
    if(state == Autonomous) {
      ...
      pid_term = controlPID(encoder[0]);
      pid_term1 = controlPID1(encoder[1]);
         
      wb_motor_set_velocity(motor[0], pid_term);
      wb_motor_set_velocity(motor[1], pid_term1);
